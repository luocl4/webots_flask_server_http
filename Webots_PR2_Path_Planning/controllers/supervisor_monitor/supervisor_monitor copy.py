...
def get_node_properties(node_webots_object):
    ...
    describe_value, ability_value = get_obj_describe(
        node_webots_object
    )  # 获取 describe 字段
    if ability_value:
    ...
    if camera_rgb and camera_rgb.getImage():
        try:
            ...
            pil_image = Image.frombytes(
                "RGBA", (camera_rgb.getWidth(), camera_rgb.getHeight()), image_data
            )
            # 将图像转换为base64字符串
    ...
    if camera_dpt and camera_dpt.getRangeImage():
        try:
            ...
            if depth_data and len(depth_data) == width * height:
                ...
                requests.post(
                    CAMERA_STATUS_URL, json={"depth_image": encoded_depth_image}
                )
            else:
                print(
                    f"Invalid depth data: expected {width * height} elements, got {len(depth_data) if depth_data else 0}"
                )
