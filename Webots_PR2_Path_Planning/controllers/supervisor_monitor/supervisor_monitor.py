...
def get_node_properties(node_webots_object):
    ...
    if Type_name == "Solid":
        ...
        if result:
            ...
            ability_value = result['ability_code'].split('/') if result['ability_code'] else []
                ...
                if origin_pos_str:
                    try:
                        ...
                        if len(origin_pos) >= 3:
                            ...
                            print(f"Supervisor INFO: 计算 {node_def_name} 位置偏移量: {offset}")
                        else:
                            solid_position_offsets[node_def_name] = [0, 0, 0]
                    except (ValueError, IndexError) as e:
                        print(f"Supervisor WARNING: 解析 {node_def_name} 原始位置失败: {e}")
                        solid_position_offsets[node_def_name] = [0, 0, 0]
                else:
                    solid_position_offsets[node_def_name] = [0, 0, 0]
        ...
        describe_value, ability_value, size_value = get_obj_describe(node_webots_object) # 获取 describe 字段
        if ability_value:
            ability_value = parse_ability(ability_value)
        obj_type = '1'
    else:
        size_value = get_obj_size(node_webots_object) # 获取 size 字段
        describe_value, ability_value, _ = get_obj_describe(node_webots_object) # 获取 describe 字段
        if ability_value:
            ability_value = parse_ability(ability_value)
        obj_type = '1'
