...
class ArmIk:
    ...
    def __init__(
        self,
        model_file="/home/luocl/Desktop/kuavo-ros-opensource/src/kuavo_assets/models/biped_s47/urdf/biped_s47.urdf",
    ...
    def _setup_collision_detection(self):
        ...
        self.__robot_left_geometries_ids = {
            "l_arm_yaw":None, "l_foream":None, "l_hand_yaw":None, "l_hand_roll":None,"l_hand_pitch":None, "l_camera":None,
        }
        self.__robot_right_geometries_ids = {
            "r_arm_yaw":None, "r_foream":None, "r_hand_yaw":None, "r_hand_roll":None,"r_hand_pitch":None, "r_camera":None,
        }
    ...
    def computeIK_left(self, q0, l_hand_pose, l_hand_RPY=None, l_norm=np.array([0, 0, 1])):
        ...
        # 添加右手关节固定约束
        for i in range(14, 21):  # 右手关节索引范围是14-20
            ik.prog().AddBoundingBoxConstraint(
    ...
    def left_hand_pose(self, q):
        ...
        # 打印从基座到末端的变换链
        frame_names = ["urdf_arm", "l_arm_pitch", "l_arm_roll", "l_arm_yaw", 
                    "l_foream", "l_hand_yaw", "l_hand_roll", "l_hand_pitch"]
    ...
    def right_hand_pose(self, q):
        ...
        # 打印从基座到末端的变换链
        frame_names = ["urdf_arm", "r_arm_pitch", "r_arm_roll", "r_arm_yaw", 
                    "r_foream", "r_hand_yaw", "r_hand_roll", "r_hand_pitch"]
        for name in frame_names:
            frame = self.__plant.GetFrameByName(name)
            pose = frame.CalcPoseInWorld(self.__plant_context)
            print(f"{name} 位置: {pose.translation()}")
...
if __name__ == "__main__":
    ...
    # 定义目标位置
    l_hand_pose = np.array([-0.0175, 0.1927, -0.4017])  # [x, y, z] 单位m
    r_hand_pose = np.array([0.43 ,-0.3 ,0.18])
