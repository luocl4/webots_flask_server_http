...
class ArmIk:
    ...
    def __init__(
        ...
        end_frames_name=[
            "urdf_arm",
            "left_gripper_robotiq_85_base_link",
            "right_gripper_robotiq_85_base_link",
        ],
        constraint_tol=9e-3,
        solver_tol=1.0e-2,
        ori_cost_weight=10,
    ...
    def calculate_eef_pose(self, joint_angles, goal_arm="both"):
        """
        ...
        # 验证输入关节角度长度（修改为6个关节）
        if goal_arm in ["left", "right"] and len(joint_angles) != 6:
            raise ValueError(f"单臂关节角度应为6个，实际提供了{len(joint_angles)}个")
        if goal_arm == "both" and len(joint_angles) != 12:
            raise ValueError(
                f"双臂关节角度应为12个（左6+右6），实际提供了{len(joint_angles)}个"
            )
        ...
        # 定义关节索引映射（根据URDF模型调整）
        joint_indices = {
            "left": 7,  # 左手关节在完整配置中的起始索引
            "right": 19,  # 右手关节在完整配置中的起始索引
        }

        # 更新目标手臂的关节角度（修改为6个）
        if goal_arm in ["left", "both"]:
            # 提取左手关节角度（若为both，前6个为左手）
            left_angles = joint_angles[:6] if goal_arm == "both" else joint_angles
            q[joint_indices["left"] : joint_indices["left"] + 6] = left_angles

        if goal_arm in ["right", "both"]:
            # 提取右手关节角度（若为both，后6个为右手）
            right_angles = joint_angles[6:] if goal_arm == "both" else joint_angles
            q[joint_indices["right"] : joint_indices["right"] + 6] = right_angles
