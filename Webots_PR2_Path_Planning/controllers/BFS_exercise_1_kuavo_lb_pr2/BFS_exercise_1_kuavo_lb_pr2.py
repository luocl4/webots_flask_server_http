...
from new_ik.ik_solve_pr2 import ArmIk
...
def id2name(robot_id, nodes_list):
    for node in nodes_list:
        if node["id"] == robot_id:
            return node["name"]
    return None
...
print(f"当前机器人名称: {robot_name}")
robot_id = name2id(robot_name, NODES_TO_MONITOR)
if robot_id is not None:
    print(f"当前机器人名称: {robot_name}, ID: {robot_id}")
else:
    print(f"未找到机器人 {robot_name} 的 ID")
...
def move_with_feedback_supervisor(
    ...
    # 获取PR2机器人节点
    pr2_name = id2name(robot_id, NODES_TO_MONITOR)
    pr2_node = supervisor.getFromDef(pr2_name)
    # print(dir(pr2_node))
    if pr2_node is None:
...
def run_arm_pick(goal_arm, robot_id):
    ...
    if goal_arm == "left" or goal_arm == "both":
        ...
        current_left = move_with_feedback_supervisor(
            ...
            grasp_left_arm_pose,
            tolerance=0.001,
            position_fields=left_arm_joint_names,
            robot_id=robot_id,
        )
    if goal_arm == "right" or goal_arm == "both":
        ...
        current_right = move_with_feedback_supervisor(
            ...
            right_arm_sensors,
            grasp_right_arm_pose,
            position_fields=right_arm_joint_names,
            tolerance=0.001,
            robot_id=robot_id,
        )
...
def run_arm_place(goal_arm, robot_id):
    # 初始化变量
    current_left = None
    current_right = None
    success_left = False
...
left_gripper_joint_names = [
    ...
    # "l_finger_gripper_motor::r_finger",
    # "l_finger_gripper_motor::l_finger_tip"
    # "l_finger_gripper_motor::r_finger_tip"
]
...
# 腰部高度控制
yao_joint_names = ["torso_lift_joint"]
...
pre_grasp_left_arm_pose_2 = [0, 0, 0, 0, 0, 0, 0]
grasp_left_arm_pose = [0, 0, 0, 0, 0, 0, 0]  # xyz[0.2374993  ,-0.33825069 , 0.10093117]
                ...
                if source == "robot_goals":
                    ...
                    if not command_data:  # 如果返回空数据，说明是停止命令
                        ...
                        if (
                            ...
                            angle_diff = (
                                (target_heading - current_heading + math.pi)
                                % (2 * math.pi)
                            ) - math.pi
                        ...
                        try:
                            ...
                            if len(new_goal) >= 2:  # 确保目标坐标至少有两个元素
                                ...
                                if is_new_goal:
                                    ...
                                    goal_yaw = new_goal_yaw
                                    last_goal_timestamp = (
                                        new_goal_timestamp  # 更新时间戳
                                    )
                                    print(
                                        f"收到新目标点: {goal}, 目标偏航角: {goal_yaw}度"
                                    )
                                    print("开始路径规划...")
                    ...
                    if goal_arm == "left" or goal_arm == "both":
                        grasp_left_arm_pose = sol_q[34:41]
                    if goal_arm == "right" or goal_arm == "both":
                        grasp_right_arm_pose = sol_q[23:30]
                    ...
                    if goal_arm == "left" or goal_arm == "both":
                        place_left_arm_pose = sol_q[34:41]
                    if goal_arm == "right" or goal_arm == "both":
                        place_right_arm_pose = sol_q[23:30]
                    ...
                    print("l_hand_pose:", l_hand_pose)
                    print("r_hand_pose:", r_hand_pose)
                    print("l_hand_rpy", l_hand_rpy)
                    print("r_hand_rpy", r_hand_rpy)
                    sol_q = arm_ik.computeIK(
                        q0, l_hand_pose, r_hand_pose, l_hand_rpy, r_hand_rpy
                    )
                    print("sol_q22222222222222:", sol_q)
                    if sol_q is None:
                        ...
                        print("grasp_left_arm_pose:", grasp_left_arm_pose)
                        print("grasp_right_arm_pose:", grasp_right_arm_pose)
                        # yao_goal_pose = [yao_goal_pose]
                        print("腰部的高度:", yao_goal_pose)
                        ...
                        current_left_rpy = []
                        current_right_rpy = []
                        # print("current_left_joint:", current_left_joint)
                        # print("current_right_joint:", current_right_joint)
                        if goal_arm == "left":
                            # 注意：此处应传入 current_left_joint
                            result = arm_ik.calculate_eef_pose(
                                yao_joint=yao_goal_pose,
                                joint_angles=current_left_joint,
                                goal_arm="left",
                            )
                            current_left_pos = result["left_position"]
                            current_left_rpy = result["left_rpy"]

                        elif goal_arm == "right":
                            result = arm_ik.calculate_eef_pose(
                                yao_joint=yao_goal_pose,
                                joint_angles=current_right_joint,
                                goal_arm="right",
                            )
                        ...
                        current_right_rpy = convert_to_list(current_right_rpy)

                        # print("current_left_pos", current_left_pos)
                        # print("current_right_pos", current_right_pos)
                        # print("current_left_rpy", current_left_rpy)
                        # print("current_right_rpy", current_right_rpy)
                        ...
                        status_info = {
                            ...
                            "current_left_rpy": current_left_rpy,
