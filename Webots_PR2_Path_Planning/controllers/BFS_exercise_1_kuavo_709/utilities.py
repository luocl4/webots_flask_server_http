...
class MyCustomRobot(Robot):
    ...
    def get_current_position(self):
        ...
        if self.gps:
            ...
            angle = self.get_current_angle_from_imu()
            shift = [math.cos(angle) * 0.2, math.sin(angle) * 0.2]  # 偏移量，避免GPS误差
            return [position[0]-shift[0], position[1]-shift[1]]  # 返回[x, y]
        return [0.0, 0.0]
    ...
    def rotate_angle_with_feedback(self, angle_radians):
        """
        使用IMU反馈信息进行高精度旋转，不包含位置补偿功能
        这个方法是rotate_angle_with_feedback的基础版本，用于避免递归调用
        
        Args:
            angle_radians: 要旋转的角度（弧度），相对于当前角度
