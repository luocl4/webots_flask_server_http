...
def move_from_to_multi_steps(current, steps, direction):
    ...
    if goal:
        ...
        try:
            ...
            current_position = map_refiner.convert_coordinates_truth(gps_position)  # 转换为地图坐标
...
def run_arm_sequence(goal_arm):
    ...
    if goal_arm == "left" or goal_arm == "both":
        ...
        set_arm_joint_positions_with_feedback(
            left_arm_motors, left_arm_sensors, grasp_left_arm_pose,
            tolerance=0.02, max_steps=80
        )
        # time.sleep(10)
                ...
                if "goal" in command_data and command_data["goal"]:
                    try:
                        ...
                        if len(new_goal) >= 2:  # 确保目标坐标至少有两个元素
                            print(new_goal)
                            new_goal = map_refiner.convert_coordinates_truth(new_goal)
                            print(new_goal)
                            if path:
                                print(f"当前路径: {path}")
        ...
        if distance_to_goal <= 3:
            ...
            if current_step_index >= len(path):
                ...
                if success:
                    # 更新位置并验证
                    gps_position = robot.get_current_position()
                    current_robot_position = map_refiner.convert_coordinates_truth(gps_position)
