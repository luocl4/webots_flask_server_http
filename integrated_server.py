...
controller_path = os.path.join(os.path.dirname(__file__), 
                             'Webots_PR2_Path_Planning', 'controllers', 'BFS_exercise_1_pr2')
sys.path.append(controller_path)
controller_path = os.path.join(os.path.dirname(__file__), 
                             'Webots_PR2_Path_Planning', 'controllers', 'BFS_exercise_1_kuavo')
sys.path.append(controller_path)
controller_path = os.path.join(os.path.dirname(__file__), 
                             'Webots_PR2_Path_Planning', 'controllers', 'BFS_exercise_1_kuavo_lb')
sys.path.append(controller_path)
controller_path = os.path.join(os.path.dirname(__file__), 
                             'Webots_capture', 'controllers', 'arm_control_leju')
sys.path.append(controller_path)
app = Flask(__name__)
CORS(app)
...
def load_scene_configs():
    ...
    config_file = os.path.join(os.path.dirname(__file__), "configs", "scene_configs.json")
    try:
        ...
        for process_pattern, process_name in process_types:
            ...
            if result.stdout.strip():
                ...
                if result.stdout.strip():
                    ...
                    logger.warning(f"发现 {len(remaining_pids)} 个残留的{process_name}进程，强制杀死")
    ...
    if 'nodes' in data and 'timestamp' in data:
        ...
        for node in data['nodes']:
            if node.get('is_moving'):
                logger.info(f"节点 '{node['name']}' (正在移动): 位置={node['position']}, 偏航角={node['rotation_degrees']['yaw']:.2f}°")
...
def update_robot_status():
    ...
    if isinstance(data.get('status'), dict):
        ...
        current_position = tuple(data['robot_position']) if 'robot_position' in data else (0, 0)
        ...
        path_planning_complete = status_dict.get('path_planning_complete', False)  # 获取路径规划完成标志
        ...
        # 如果是初次接收到路径规划结果，记录日志
        if path_planning_complete and not robot_goals[object_id].get('path_planning_logged', False):
            robot_goals[object_id]['path_planning_logged'] = True
            if is_reachable:
                logger.info(f"路径规划已完成且成功: 机器人(ID:{object_id}), 总段数:{total_segments}")
            else:
                logger.warning(f"路径规划已完成但失败: 机器人(ID:{object_id}), 错误:{error_message}")
        ...
        logger.info(f"机器人状态更新(新格式): ID={object_id}, 位置={current_position}, 状态='{current_status}', 进度={current_segment}/{total_segments}")
    ...
    # 处理旧格式的状态数据 - 向后兼容
        ...
        if isinstance(current_status, str) and ("Goal reached" in current_status or "到达目标" in current_status):
            # 移动完成，更新为状态3
            robot_goals[object_id]["status"] = 3
            robot_status_dict[object_id] = 3
        ...
        logger.info(f"机器人状态更新(旧格式): ID={object_id}, 位置={current_position}, 状态='{current_status}'")
...
def move_set_goal():
    ...
    try:
        ...
        logger.info(f"[set_robot_goal] 设置机器人(ID:{object_id})目标位置: {goal_pos}, 角度: {goal_angle}°")
        request_response_log.append({
            "event_type": "set_goal",
            "object_id": object_id,
            "goal_pos": goal_pos,
            ...
            if object_id in robot_goals:
                if robot_goals[object_id].get('path_planning_complete', False):
                    if robot_goals[object_id].get('is_reachable') is False:
                        # 路径规划失败，目标不可达
                        error_message = robot_goals[object_id].get('error_message', f"无法找到到达目标点 {goal_pos} 的可行路径")
                        logger.warning(f"[set_robot_goal] 机器人(ID:{object_id})路径规划失败: {error_message}")
                    ...
                    # 路径规划已完成且成功
                    logger.info(f"[set_robot_goal] 机器人(ID:{object_id})路径规划成功，总段数: {robot_goals[object_id].get('total_segments', 0)}")
                    return jsonify({
                        "code": 200, 
                        "message": "success", 
                        "total_segments": robot_goals[object_id].get('total_segments', 0),
                        "is_reachable": True
                    }), 200
                ...
                # 兼容旧版本：检查is_reachable标志和total_segments数量
                    ...
                    # 路径规划失败，目标不可达（旧版本兼容）
                    error_message = robot_goals[object_id].get('error_message', f"无法找到到达目标点 {goal_pos} 的可行路径")
                    logger.warning(f"[set_robot_goal] 机器人(ID:{object_id})路径规划失败(旧版本检测): {error_message}")
                ...
                # 检查是否有段数信息（表示路径规划可能已成功）
                    ...
                    # 路径规划可能已成功（旧版本兼容）
                    logger.info(f"[set_robot_goal] 机器人(ID:{object_id})路径规划成功(旧版本检测)，总段数: {robot_goals[object_id].get('total_segments', 0)}")
                    return jsonify({
                        "code": 200, 
                        "message": "success", 
                        "total_segments": robot_goals[object_id].get('total_segments', 0),
    ...
    try:
        ...
        if object_id in robot_goals:
            ...
            response_data.update({
                "process": goal_state.get('process', '0/0')
            })
...
def move_check_pos():
    ...
    try:
        ...
        if not target_object:
            return jsonify({"code": 400, "message": f"未找到ID为{object_id}的物体"}), 400
...
def print_status():
    ...
    while True:
        try:
            ...
            scene_name = current_loaded_scene.get('name', 'None') if current_loaded_scene else 'None'
            
            print(f"\n[{current_time}] 服务器状态:")
            print(f"├─ 当前场景: {scene_name}")
            print(f"├─ Webots进程: {'运行中' if webots_process and webots_process.poll() is None else '未运行'}")
            print(f"└─ 服务器运行正常")
