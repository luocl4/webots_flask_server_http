...
int main(int argc, char **argv)
{
  ...
  while (wb_robot_step(TIME_STEP) != -1)
  {
    if (counter <= 0)
    {
      switch (state)
      {
      case WAITING: // WAITING: 机器人等待物体接近。当距离传感器的值小于 500 时，切换到 GRASPING 状态。
        ...
        break;
      case GRASPING: // GRASPING: 机器人开始抓取物体，三个手指电机移动到位置 0.85。
        for (i = 0; i < 4; ++i)
          wb_motor_set_position(ur_motors[i], target_positions[i]);
        printf("Rotating arm\n");
        state = ROTATING;
        break;
      case ROTATING: // ROTATING: 机器人旋转手臂到目标位置，使用 target_positions 数组中定义的位置。
        ...
        break;
      case RELEASING: // RELEASING: 机器人释放物体，将手指电机移动到最小位置。
        for (i = 0; i < 4; ++i)
          wb_motor_set_position(ur_motors[i], 0.0);
        printf("Rotating arm back\n");
        state = ROTATING_BACK;
        break;
      case ROTATING_BACK: // ROTATING_BACK: 机器人将手臂旋转回原位。如果手腕传感器的值大于 -0.1，切换回 WAITING 状态，重新等待物体。
